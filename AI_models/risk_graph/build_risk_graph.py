...
On real data, "overdue" scope alone can still be 50-100+ invoices, which
renders as an unreadable hairball rather than the memorable causal chain
this graph is meant to be. max_focus_invoices caps it to the invoices with
the largest cash exposure (the ones actually threatening the buffer) -
the rest are still counted in summary.total_overdue_invoice_count so
...
def build_risk_graph(
    ...
    include_shap=True,
    max_focus_invoices=12,  # caps node count so the graph stays demo-legible
):
    ...
    total_focus_count = len(focus)
    if max_focus_invoices is not None and len(focus) > max_focus_invoices:
        focus = focus.sort_values("invoice_amount", ascending=False).head(max_focus_invoices)
    ...
    for row in focus.itertuples(index=False):
        ...
        nodes.append({
            ...
            # Model 1's own cold-start signal (fewer than
            # MIN_HISTORY_FOR_CONFIDENCE closed invoices for this customer) -
            # "low" means this invoice's P10/50/90 leaned on a sector/global
            # prior rather than this customer's actual payment history, so
            # the graph should say so rather than presenting it as certain.
            "confidence": getattr(row, "confidence", "normal"),
        })
    ...
    return {
        ...
        "summary": {
            ...
            "total_overdue_invoice_count": total_focus_count,
