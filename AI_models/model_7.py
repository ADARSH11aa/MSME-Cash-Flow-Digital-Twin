...
# MODEL 7 WEIGHTS
#
# "liquidity" used to be weighted 0.40 with no counterweight for whether an
# option takes on debt, so financing options (which have inflated
# gap_reduction numbers) always beat free, non-debt options regardless of
# cost or risk - the opposite of what a "non-debt-first" ranker should do.
# "debt" is a separate, explicit dimension so that preference is visible
# and tunable rather than an accidental side effect of the liquidity weight.
...
def calculate_debt_score(is_debt):
    """Non-debt options score higher, so free/commercial actions are not
    automatically outranked by financing options with larger raw liquidity
    numbers."""

    return 100.0 if not is_debt else 30.0
...
def generate_explanation(
    ...
    explanations = {
        ...
        "debt": "It avoids taking on new debt or financing."
    }
...
if __name__ == "__main__":
    # Windows' console defaults to cp1252, which can't encode the ₹ sign
    # main() prints below - reconfigure stdout to UTF-8 rather than
    # stripping the currency symbol. Only done here (not at import time),
    # so importing rank_recovery_options() elsewhere (e.g. the backend
    # gateway) never touches the caller's own stdout encoding.
    import sys
